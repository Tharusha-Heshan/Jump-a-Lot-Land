...
// Variables for Meteors and Stars
struct Meteor {
    float x;
    float y;
    float speed;
...
void updateLevel3(float dt) {
    ...
    if(platform1X > 300) platform1Speed = -1;
    if(platform1X < 100) platform1Speed = 1;

    platform2X += platform2Speed;
    if(platform2X > 500) platform2Speed = -1.2f;
    if(platform2X < 200) platform2Speed = 1.2f;

    platform3X += platform3Speed;
    if(platform3X > 650) platform3Speed = -1.5f;
    if(platform3X < 400) platform3Speed = 1.5f;

    portalTime += dt;
    portalY = portalBaseY + sinf(portalTime * 2.0f) * 40.0f;
...
bool checkPlatformCollision(float x, float y, float width, float height) {
    if(x + width > platform1X && x < platform1X + TILE_SIZE * 2 &&
       y >= platform1Y + TILE_SIZE - 5 && y <= platform1Y + TILE_SIZE + 10) return true;

    if(x + width > platform2X && x < platform2X + TILE_SIZE * 2 &&
       y >= platform2Y + TILE_SIZE - 5 && y <= platform2Y + TILE_SIZE + 10) return true;

    if(x + width > platform3X && x < platform3X + TILE_SIZE * 2 &&
       y >= platform3Y + TILE_SIZE - 5 && y <= platform3Y + TILE_SIZE + 10) return true;
...
void drawMoonWorld() {
    ...
    for(int i = 0; i < 360; i++) {
        ...
        glVertex2f(520 + cos(a) * 110, 320 + sin(a) * 110);
    }
    glEnd();
    ...
    for(int i = 0; i < 360; i++) {
        float a = i * 3.14159f / 180.0f;
        glVertex2f(520 + cos(a) * 80, 320 + sin(a) * 80);
    }
    glEnd();
    ...
    for(int i = 0; i < 360; i++) {
        float a = i * 3.14159f / 180.0f;
        glVertex2f(500 + cos(a) * 12, 340 + sin(a) * 12);
    }
    glEnd();
    ...
    for(int i = 0; i < 360; i++) {
        float a = i * 3.14159f / 180.0f;
        glVertex2f(545 + cos(a) * 8, 300 + sin(a) * 8);
    }
    glEnd();
    ...
    for(int i = 0; i < 360; i++) {
        float a = i * 3.14159f / 180.0f;
        glVertex2f(540 + cos(a) * 6, 355 + sin(a) * 6);
    }
    glEnd();
    ...
    glEnd(); // FIXED: Added missing glEnd() here!
    ...
    for(int i = 0; i < METEOR_COUNT; i++) {
        ...
        for(int j = 0; j < 12; j++) {
            ...
            glVertex2f(mx + 10 + cos(a) * 10, my + 10 + sin(a) * 10);
        }
        glEnd();
        ...
        for(int j = 0; j < 12; j++) {
            float a = j * 6.28318f / 12.0f;
            glVertex2f(mx + 8 + cos(a) * 5, my + 12 + sin(a) * 5);
        }
        glEnd();
    }
    ...
    for(int i = 0; i < 40; i++) {
        ...
        glVertex2f(portalX + 20 + cos(a) * 40, portalY + 40 + sin(a) * 60);
    }
    glEnd();
    ...
    for(int i = 0; i < 40; i++) {
        float a = i * 6.28318f / 40.0f;
        glVertex2f(portalX + 20 + cos(a) * 18, portalY + 40 + sin(a) * 35);
    }
    glEnd();
    ...
    for(int i = 0; i < 40; i++) {
        float a = i * 6.28318f / 40.0f;
        glVertex2f(portalX + 20 + cos(a) * 10, portalY + 40 + sin(a) * 20);
    }
    glEnd();
}
